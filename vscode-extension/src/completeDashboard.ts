...
export class CompleteDashboard {
  public static currentPanel: CompleteDashboard | undefined;
  private readonly _panel: vscode.WebviewPanel;
  private readonly _extensionUri: vscode.Uri;
  private readonly _manager: CloakxManager;
  private _disposables: vscode.Disposable[] = [];
  private _refreshInterval: NodeJS.Timeout | undefined;

  public static createOrShow(extensionUri: vscode.Uri, manager: CloakxManager) {
    ...
    const panel = vscode.window.createWebviewPanel(
      'cloakxCompleteDashboard',
      '🔐 Cloakx - Secret Manager',
  ...
  private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, manager: CloakxManager) {
    this._panel = panel;
    ...
    try {
      switch (command) {
        ...
        case 'deleteSecret':
          ...
          const confirmed2 = await vscode.window.showWarningMessage(
            `Delete secret "${deleteSecret.key}"?`,
            'Delete',
            'Cancel'
          );
...
<body>
  ...
  private getSecretsPage(secrets: any[], environments: string[], currentEnv: string): string {
            ...
            <div class="secret-meta">
              ...
              ${
                secret.tags && secret.tags.length > 0
                  ? secret.tags.map((t: string) => `<span class="tag">#${t}</span>`).join('')
                  : ''
              }
